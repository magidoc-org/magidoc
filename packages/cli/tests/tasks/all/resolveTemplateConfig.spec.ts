...
describe('resolving template config', () => {
  ...
  const templateConfig: RawMagidocTemplateConfig = {
    ...
    SUPPORTED_OPTIONS: [templates.APP_FAVICON, templates.PAGES] as ReadonlyArray<Variable<unknown>>,
  }

  beforeEach(() => {
    vi.mocked(loadTemplateConfig).mockReturnValueOnce(Promise.resolve(templateConfig))
  })
  ...
  describe('task is enabled', () => {
    ...
    it('should set the resolved template configuration properly', async () => {
      ...
      expect(ctx.templateConfiguration).toEqual({
        ...
        schemaTargetLocation: path.join(ctx.tmpDirectory.path, templateConfig.SCHEMA_TARGET_LOCATION),
        staticAssetsLocation: path.join(ctx.tmpDirectory.path, templateConfig.STATIC_ASSETS_LOCATION),
        envFileLocation: path.join(ctx.tmpDirectory.path, templateConfig.ENV_FILE_LOCATION),
      })
    })
  })
})
