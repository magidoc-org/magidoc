...
global.getTestSchema = () => buildClientSchema(schema as unknown as IntrospectionQuery)

global.getMandatoryField = (type: Maybe<GraphQLObjectType>, name: string) => {
  const result = type?.getFields()[name]

  if (!result) {
    throw new Error(`field ${name} should not be null... did you modify the test schema?`)
  }
