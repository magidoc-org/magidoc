...
When initializing a template, you may choose one of the [Magidoc templates](/templates/introduction). Magidoc installs the template with the [Pnpm](https://pnpm.io/workspaces) version bundled with the CLI, so nothing needs to be installed separately and the template always resolves the exact dependency versions it was released with.
