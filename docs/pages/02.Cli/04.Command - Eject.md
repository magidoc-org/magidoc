...
When initializing a template, you may choose one of the [Magidoc templates](/templates/introduction). You also have the option to choose a package manager. By default, Magidoc uses the [Pnpm](https://pnpm.io/workspaces) version bundled with the CLI, which does not need to be installed separately. It is the only option that honors the lockfile shipped with the template, so any other package manager resolves transitive dependencies fresh and may pick up a broken upstream release. Use `system-pnpm` to run the Pnpm from your `PATH` instead.
